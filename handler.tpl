 ...
 * {{ .Filename }}
 */
func (s *ServiceV3) {{ .Name }}(w http.ResponseWriter, r *http.Request{{ if gt (len .Params) 1 }}, body {{ if eq .Cmd ":one" }}sqlcoa3gen.{{ .Name }}Params{{ else if eq .Cmd ":many" }}sqlcoa3gen.{{ .Name }}Inline{{ end }}{{ end }}) (*sqlcoa3gen.{{ .Name }}Return, error) {
